...
/**
 * This plugin is used to access maor files
 *
 * @since 2.0
 * @package    repository_maor
...
/**
 * repository_maor is used to search merlot.org in moodle
 *
 * @since 2.0
 * @package    repository_maor
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class repository_maor extends repository {

    public function __construct($repositoryid, $context = SYSCONTEXTID, $options = array()) {
        parent::__construct($repositoryid, $context, $options);
        $this->keyword = optional_param('maor_keyword', '', PARAM_RAW);
        $this->author = optional_param('maor_author', '', PARAM_RAW);
        $this->licensekey = trim(get_config('maor', 'licensekey'));
    }
    ...
    private function _get_collection($keyword) {
        ...
        // Send query to Israeli Maor servers for logging
        $maor_response = file_get_contents('http://maor.iucc.ac.il/materials.php?keywords='. urlencode($keyword) );
    ...
    public static function type_config_form($mform, $classname = 'repository') {
        ...
        $licensekey = get_config('maor', 'licensekey');
        if (empty($licensekey)) {
            $licensekey = '';
        }
        $strrequired = get_string('required');
        $mform->addElement('text', 'licensekey', get_string('licensekey', 'repository_maor'), array('value'=>$licensekey,'size' => '40'));
